...
class ExitSignalManager:
    """Dedicated exit signal management for 0DTE trades - FIXED VERSION"""
    ...
    def get_exit_signals(self, entry_decision: str, entry_price: float, entry_time: datetime, 
        ...
        if trade_type == 'STOCK':
            # For stock trades, get SPY current price
            ...
            if strike:
                ...
                if current_option_price:
                    ...
                    # Fallback: Estimate based on SPY movement and time decay
                ...
                # No strike provided, estimate option value
                current_price, price_source = self._estimate_option_value(
                    entry_price, entry_decision, entry_time, targets
                )
                exit_signals['calculation_method'] = 'Estimated Options Value'
        ...
        if current_price > 0:
            ...
            pnl_pct = -100  # Assume total loss if can't get price
        ...
        # For options, targets refer to underlying SPY price
    ...
    def _estimate_option_value(self, entry_price: float, entry_decision: str, 
        ...
        try:
            ...
            time_decay_factor = max(0.1, 1 - (time_in_trade * 0.15))  # Rough theta estimate
            
            # Moneyness factor
            if 'LONG' in entry_decision:
                # Call option estimate
                upside_target = targets.get('upside_target', spy_price * 1.005)
                if spy_price >= upside_target:
                    intrinsic_multiplier = 2.5  # ITM call gained value
                elif spy_price >= upside_target * 0.5:
                    intrinsic_multiplier = 1.2  # Moving toward target
                else:
                    intrinsic_multiplier = 0.6  # Moving away from target
            else:
                # Put option estimate
                downside_target = targets.get('downside_target', spy_price * 0.995)
                if spy_price <= downside_target:
                    intrinsic_multiplier = 2.5  # ITM put gained value
            ...
            estimated_value = max(0.01, estimated_value)  # Minimum $0.01
...
class GapTradingAnalyzer:
    ...
    def get_spy_data_enhanced(self) -> Dict:
        """Get SPY data with VWAP debugging"""
        spy_data = {
            'current_price': 0,
            'historical': None,
            'volume_data': {},
        ...
        try:
            ...
            if spy_quote and spy_quote.get('last'):
                ...
                if tradier_hist and 'history' in tradier_hist:
                    ...
                    # Calculate VWAP from Tradier data with debugging
                    if hist_data:
                        try:
                            total_pv = 0
                            total_volume = 0
                            ...
                            spy_data['debug_info'].append(f"Valid VWAP points: {valid_points}, Total Volume: {total_volume:,.0f}")
                            ...
                            if total_volume > 0 and valid_points > 0:
                                ...
                                spy_data['debug_info'].append("No valid volume data for VWAP calculation")
        ...
        try:
            ...
            if not spy_hist.empty:
                if spy_data['current_price'] == 0:  # Only if we don't have current price yet
                ...
                spy_data['debug_info'].append(f"Yahoo data: {len(spy_hist)} minutes of data")
                ...
                if total_volume > 0:
                    ...
                    spy_data['debug_info'].append(f"Yahoo VWAP: ${vwap:.2f}, Distance: {vwap_distance:.3f}%")
                    return spy_data
                else:
                    spy_data['debug_info'].append("No volume data in Yahoo response")

            else:
                spy_data['debug_info'].append("Yahoo returned empty dataset")
        ...
        spy_data.update({
            ...
            'vwap_data': {
                'vwap': proxy_data['current_price'],  # This causes 0% distance!
                'distance_pct': 0.000,
        ...
        spy_data['debug_info'].append("Using proxy data - VWAP = current price (0% distance)")
...
def display_exit_signals_ui(exit_manager, entry_data=None):
    ...
    if not active_trade:
        ...
        *Use the form below to input your trade details*
            ...
            if submitted:
                if entry_price <= 0:
                    ...
                    trade_record = {
                        ...
                        'targets': {
                            'upside_target': 0,    # Auto-calculated
                            'downside_target': 0   # Auto-calculated  
                        }
                    }
        ...
        # DISPLAY ACTIVE TRADE WITH PERSISTENT TRACKING
    ...
    # TRADE MANAGEMENT BUTTONS (Always show if active trade exists)
    if st.session_state.get('active_trade'):
        st.markdown("---")
        col1, col2, col3 = st.columns(3)
        
        with col1:
            if st.button("✅ Close Trade (Profit)", type="primary"):
                # Move to history with profit flag
                closed_trade = st.session_state['active_trade'].copy()
                closed_trade['closed_at'] = datetime.now()
                closed_trade['close_reason'] = 'PROFIT'
                st.session_state['active_trade'] = None
                st.success("✅ Trade closed with profit!")
                st.rerun()
                
        with col2:
            if st.button("🛑 Close Trade (Loss)", type="secondary"):
                # Move to history with loss flag  
                closed_trade = st.session_state['active_trade'].copy()
                closed_trade['closed_at'] = datetime.now()
                closed_trade['close_reason'] = 'LOSS'
                st.session_state['active_trade'] = None
                st.warning("Trade closed with loss.")
                st.rerun()
                
        with col3:
            if st.button("🗑️ Clear Tracking", type="secondary"):
                st.session_state['active_trade'] = None
...
def main():
    ...
    if auto_refresh and trading_window_ok:
        # Only refresh if no active trade, or add user confirmation
        if st.session_state.get('active_trade') is None:
            time.sleep(30)
            st.rerun()
        else:
            st.warning("⚠️ Auto-refresh disabled while tracking active trade")
            if st.button("🔄 Manual Refresh (keeps trade)", type="secondary"):
                st.rerun()
...
if __name__ == "__main__":
    main()    initial_sidebar_state="expanded"
)
...
if __name__ == "__main__":
    main()        # Check per-symbol rate limit
        if symbol in self.last_request:
            time_since_last = current_time - self.last_request[symbol]
            if time_since_last < self.min_interval:
                return False
